...
#  $@
	...
	"start")
		...
		if _is_function process_update; then 
			...
			nohup "${SCRIPT}" "startbot" "$2" "${SESSION}" &>>/home/t.mukhametshin/telegram-bot-bash/startbot.log &
#			echo "type: "$(type -t $1)
#        	        echo "2: "$2
#			echo "after nohup:"
#			ps -ef | grep "${SCRIPT}"
		...
		ps -fu "${UID}" | grep -F "$1" | grep -v ' grep'| grep -F "${ME}" | sed 's/\s\+/\t/g' | cut -f 2
		echo "ps -fu ${UID} | grep -F $1 | grep -v ' grep'| grep -F ${ME} | sed 's/\s\+/\t/g' | cut -f 2"
		echo "$$ - $PPID -- $(proclist ${SESSION})"
